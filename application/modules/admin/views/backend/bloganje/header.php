...
<head>
	...
	<?php endif;?>
	<script type="text/javascript" src="<?php echo $this->config->item('base_url') ?>assets/bloganje/js/jquery.js"></script>
	<script type="text/javascript" src="<?php echo $this->config->item('base_url') ?>assets/bloganje/js/jquery.simpletip.js"></script>
	<script>
		var StyleFile = "theme" + document.cookie.charAt(6) + ".css";
		document.writeln('<link rel="stylesheet" type="text/css" href="<?php echo $this->config->item('base_url') ?>
			application / views / backend / bloganje / css / ' + StyleFile + '">');
			$().ready( function(){
	...
	<!--[if IE]>
	<link rel="stylesheet" type="text/css" href="<?php echo $this->config->item('base_url') ?>application/modules/admin/views/backend/bloganje/css/ie-sucks.css" />
	<![endif]-->
</head>
<body>
		...
		<div id="top-panel">
			<div id="panel">
				...
				<?php if ( $this->backend->have_access( $action, 'active', false ) && $active_count > 0) : ?>
					<li><?php echo anchor('admin/active/' . $action, lang('active_record') . ' (' . $active_count . ')', 'class="report"');
					//TODO: add records need to active
 ?></li>
				<?php endif;?>
...
if (!empty($action) && $me -> option -> attributes() -> search == 1) {
	include APPPATH . 'modules/admin/views/backend/bloganje/search_block.php';
}
